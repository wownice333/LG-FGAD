...
class Server():
    def __init__(self, encoder, discriminator, device):
        self.encoder = encoder.to(device)
        self.discriminator = discriminator.to(device)
        self.W_enc = {key: value for key, value in self.encoder.named_parameters()}
        self.W_dis = {key: value for key, value in self.discriminator.named_parameters()}
        self.model_cache = []
    ...
    def aggregate_weights(self, selected_clients):
        ...
        for client in selected_clients:
            ...
            # for k in self.W_dis.keys():
            #     parameter_num += (client.W_stu[k].numel())
            # for k in self.W_enc.keys():
            #     parameter_num += (client.W[k].numel())
        # print(sum(p.numel() for p in self.model.parameters())*len(selected_clients))
        # print(len(self.W.keys())*len(selected_clients))
        # print(parameter_num)
        for k in self.W_dis.keys():
            self.W_dis[k].data = torch.div(torch.sum(
                torch.stack([torch.mul(client.W_stu[k].data, client.train_size) for client in selected_clients]),
                dim=0), total_size).clone()
        for k in self.W_enc.keys():
            self.W_enc[k].data = torch.div(
